...
func SetUp(name, kVersion string, installServing, installEventing bool) error {
	...
	if err := createMinikubeCluster(); err != nil {
		return fmt.Errorf("error creating minikube cluster: %w", err)
	}
	fmt.Print("\n")
	...
	if installKnative {
		if installServing {
			if err := install.Serving(""); err != nil {
				return fmt.Errorf("error installing serving to minikube cluster %s: %w", clusterName, err)
			}
			if err := install.Kourier(); err != nil {
				return fmt.Errorf("error installing kourier to minikube cluster %s: %w", clusterName, err)
			}
			if err := install.KourierMinikube(); err != nil {
				return fmt.Errorf("error configuring kourier for minikube cluster %s: %w", clusterName, err)
			}
		}
		if installEventing {
			if err := install.Eventing(); err != nil {
				return fmt.Errorf("error installing eventing to minikube cluster %s: %w", clusterName, err)
			}
		}
	}
...
func createMinikubeCluster() error {
	if err := checkMinikubeVersion(); err != nil {
		return fmt.Errorf("error checking minikube version: %w", err)
	}
	if err := checkForExistingCluster(); err != nil {
		return fmt.Errorf("error handling or checking for existing minikube cluster: %w", err)
	}
	return nil
}
...
func checkMinikubeVersion() error {
	...
	if err != nil {
		return fmt.Errorf("unable to check minikube version with error: %w", err)
	}
	fmt.Printf("Minikube version is: %s\n", string(out))

	userMinikubeVersion, err := parseMinikubeVersion(string(out))
	if err != nil {
		return fmt.Errorf("error parsing minikube version: %w", err)
	}
...
func checkForExistingCluster() error {
	...
	if err != nil {
		...
		if !strings.Contains(string(out), "MK_USAGE_NO_PROFILE") {
			return fmt.Errorf("error checking existing minikube profiles: %w", err)
		}
	}
	...
	if matches {
		...
		if strings.ToLower(resp) != "y" {
			...
			if err != nil {
				...
				return fmt.Errorf("error getting existing kubernetes namespaces for minikube cluster %s: %w", clusterName, err)
			}
...
func recreateCluster() error {
	...
	if err := deleteCluster.Run(); err != nil {
		return fmt.Errorf("error deleting minikube cluster %s: %w", clusterName, err)
	}
	if err := createNewCluster(); err != nil {
		return fmt.Errorf("%w", err)
	}
